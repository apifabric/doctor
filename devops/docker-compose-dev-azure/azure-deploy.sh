...
resourcename="doctor"  # lower case, only
resourcegroup="doctor_rg"
...
# git clone https://github.com/apilogicserver/doctor.git
# cd doctor
...
az group create --name doctor_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group doctor_rg --sku S1 --is-linux
az container create --resource-group doctor_rg --name doctor --image apilogicserver/doctor:latest --dns-name-label doctor --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//doctor.westus.azurecontainer.io:5656'
